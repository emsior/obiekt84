...
**Wejście** (klawiatura i mysz) obsługuje wyłącznie `simulation_runner.gd` w `_unhandled_input`. W fazie PLAN zamienia piksele na komórki i woła `begin_drag` / `drag_to` / `end_drag` edytora; edytor nie widzi `InputEvent`. Rdzeń nigdy nie widzi inputu. Korzeń HUD i pełnoekranowe tło `Background` mają `mouse_filter = IGNORE`, żeby nie przechwytywały kliknięć nad planszą i osią czasu. Pilnuje tego `test_board_mouse_input_reaches_runner_through_viewport`, który wysyła zdarzenia przez `Viewport.push_input`, czyli najpierw przez GUI, a dopiero potem do `_unhandled_input`.
